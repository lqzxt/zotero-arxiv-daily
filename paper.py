    ...
    def tex(self) -> Optional[dict[str, str]]:
        ...
        with ExitStack() as stack:
            ...
            try:
                ...
                logger.warning(f"Failed to download source for {self.arxiv_id}: {e}")
                return None
            except Exception as e:
                logger.warning(f"Unexpected error downloading TeX source for {self.arxiv_id}: {e}")
                return None
            ...
            match len(bbl_file):
                case 0:
                    if len(tex_files) > 1:
                        logger.debug(f"Cannot find main tex file of {self.arxiv_id}: multiple tex files, no bbl.")
                        main_tex = None
                    else:
                ...
                case 1:
                    ...
                    if main_tex not in tex_files:
                        logger.debug(f"Cannot find main tex file of {self.arxiv_id}: bbl mismatch.")
                        main_tex = None
                case _:
                    logger.debug(f"Cannot find main tex file of {self.arxiv_id}: multiple bbls.")
                    main_tex = None
            if main_tex is None:
                logger.debug(f"Trying to choose tex file containing \\begin{{document}} for {self.arxiv_id}")
            file_contents = {}
            for t in tex_files:
                f = tar.extractfile(t)
            ...
            if main_tex is not None:
                ...
                logger.debug(f"No tex file containing \\begin{{document}} for {self.arxiv_id}")
